...
inline bool is_leaf(int x) { return degree[x] == 1; }

bool check_valid() {
    for (int i = 2; i < dia_length; i++) {
        for (int j = head[dia[i]]; j; j = link[j].nxt) {
            int v = link[j].x;
            if (v != dia[i-1] && v != dia[i+1]) {
                if (!is_leaf(v)) return false; // connected to the diameter and not a leaf
            }
        }
    }
    return true;
